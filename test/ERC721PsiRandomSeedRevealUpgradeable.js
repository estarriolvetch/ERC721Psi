...
describe('ERC721PsiRandomSeedRevealUpgradeable', function () {
  beforeEach(async function () {
    ...
    this.ERC721PsiImplementation = await ethers.getContractFactory('ERC721PsiRandomSeedRevealUpgradeableMock');
  ...
  context('with minted tokens', async function () {
    ...
    describe('seed', async function () {
      ...
      context('Reveal the seed', function () {
        ...
        it('seed query for nonexistent token', async function () {
          await expect(this.ERC721Psi.seed('7')).to.be.revertedWith(
            "ERC721PsiRandomSeedReveal: seed query for nonexistent token"
          )
        });
      });
